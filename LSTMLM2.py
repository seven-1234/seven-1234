...
class ChangeLSTM(nn.Module):
    def __init__(self, input_sz, hidden_sz):
        ...
        self.W2 = nn.Parameter(torch.Tensor(hidden_sz, hidden_sz * 4))
        self.U2 = nn.Parameter(torch.Tensor(hidden_sz, hidden_sz * 4))
        self.bias2 = nn.Parameter(torch.Tensor(hidden_sz * 4))
        self.init_weights()
    ...
    def forward(self, x, init_states=None):
        ...
        if init_states is None:
            h_t, c_t ,h_t2 , c_t2= (torch.zeros(bs, self.hidden_size).to(x.device),
        ...
        for t in range(seq_sz):
            ...
            x_t2 = h_t
            gates2 = x_t2 @ self.W2 + h_t2 @ self.U2 + self.bias2
            i_t2, f_t2, g_t2, o_t2 = (
                torch.sigmoid(gates2[:, :HS]),
                torch.sigmoid(gates2[:, HS:HS * 2]),
                torch.tanh(gates2[:, HS * 2:HS * 3]),
                torch.sigmoid(gates2[:, HS * 3:]),
            )
...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        # 调用ChangeLSTM
        self.LSTM = ChangeLSTM(emb_size, n_hidden)
